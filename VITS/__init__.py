...
model = "..\..\TTSModels\ATRI.pth"
config = ".\plugins\gal_voice_bot\VITS\models\ATRI\config.json"
...
model2 = "..\..\TTSModels\Yuzu.pth"
config2 = ".\plugins\gal_voice_bot\VITS\models\Yuzu\config.json"
...
def Trans2(text="お兄ちゃん大好き", speaker_id=4, out_path='voice.wav'):
    ...
    with no_grad():
        ...
        x_tst_lengths = LongTensor([stn_tst.size(0)])
        sid = LongTensor([speaker_id])
        audio = net_g_ms2.infer(x_tst, x_tst_lengths, sid=sid, noise_scale=.667,
                                noise_scale_w=0.8, length_scale=1)[0][0, 0].data.cpu().float().numpy()
