...
def yozo_func(msg,name):
    Trans2(msg,yuzu_dict[name],'voice.wav')
    voice=f'file:///'+os.getcwd()+'/voice.wav'
    return MessageSegment.record(voice)
...
genshin_regex = "^(派蒙|凯亚|安柏|丽莎|琴|香菱|枫原万叶|迪卢克|温迪|可莉|早柚|托马|芭芭拉|优菈|云堇|钟离|魈|凝光|雷电将军|北斗|甘雨|七七|刻晴|神里绫华|雷泽|神里绫人|罗莎莉亚|阿贝多|八重神子|宵宫|荒泷一斗|九条裟罗|夜兰|珊瑚宫心海|五郎|达达利亚|莫娜|班尼特|申鹤|行秋|烟绯|久岐忍|辛焱|砂糖|胡桃|重云|菲谢尔|诺艾尔|迪奥娜|鹿野院平藏)(发送|说|\ )(.+)$"
...
async def _(bot:Bot,event: MessageEvent,matched: Tuple[Any, ...] = RegexGroup()):
    ...
    if cmd=='发送':
        ...
        try:
            ...
            await yuzu_cmd.finish('发送成功')
        except ActionFailed as e:
            await yuzu_cmd.finish('API调用失败：' + str(e) + '。请检查输入字符是否匹配语言。')
    else:
        try:
            if(gp):
                await yuzu_cmd.finish(message=yozo_func(msg=msg,name=name))
            else:
                await yuzu_cmd.finish(message=yozo_func(msg=msg,name=name))
        except ActionFailed as e:
            await yuzu_cmd.finish('API调用失败：' + str(e) + '。请检查输入字符是否匹配语言。')
...
async def _(bot:Bot,event: MessageEvent,matched: Tuple[Any, ...] = RegexGroup()):
    ...
    if cmd=='发送':
        ...
        try:
            ...
            await genshin_cmd.finish('发送成功')
        except ActionFailed as e:
            await genshin_cmd.finish('API调用失败：' + str(e) + '。请检查输入字符是否匹配语言。')
    else:
        try:
            if(gp):
                await genshin_cmd.finish(message=gs_func(msg=msg, name=name))
            else:
                await genshin_cmd.finish(message=gs_func(msg=msg, name=name))
        except ActionFailed as e:
            await genshin_cmd.finish('API调用失败：' + str(e) + '。请检查输入字符是否匹配语言。')
